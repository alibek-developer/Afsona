...
function MiniCalendar({
	...
	const monthName = view.toLocaleDateString('uz-UZ', {
		month: 'long',
		year: 'numeric',
	})
	...
	return (
		<div className='rounded-2xl border border-border bg-card/60 backdrop-blur-md p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)]'>
			<div className='mb-4 flex items-center justify-between'>
				...
				<div className='font-display text-base font-bold capitalize tracking-tight'>
					{monthName}
				</div>
				<button
					type='button'
		...
		<div className='relative w-[80px] h-[70px] mx-auto mt-2 mb-4 flex items-center justify-center'>
			{isRoom ? (
				<>
					...
					<div
						className={`w-[60px] h-12 rounded-lg ${tableColor} z-10 flex flex-col items-center justify-start pt-1 shadow-sm`}
					>
						{isVIP && !isSelected && (
							<span className='text-white/80 text-[8px] font-black tracking-widest mt-1'>
								VIP
							</span>
						)}
					</div>
				...
				<>
					...
					<div
						className={`w-[46px] h-[46px] rounded-[14px] ${tableColor} z-10 flex items-center justify-center shadow-sm`}
					>
						{isVIP && !isSelected && (
							<span className='text-white/80 text-[8px] font-black tracking-widest relative -top-2'>
								VIP
							</span>
						)}
					</div>
...
export default function RestaurantReservation() {
	...
	// Filters
	const [seatingFilter, setSeatingFilter] = useState<'all' | 'xona' | 'stol'>(
		'all',
	)
		...
		try {
			...
			const tablesWithStatus = (tablesData || []).map((table: TableItem) => {
				const activeOrder = activeOrders?.find(
					(order: Order) => order.table_id === table.id,
				)
				return {
					...table,
			...
			const floors = Array.from(
				new Set(tablesWithStatus.map(t => t.floor)),
			).sort()
	...
	const submitBooking = async (e: React.FormEvent) => {
		...
		try {
			...
			toast.error(
				'Bron qilishda xatolik yuz berdi. Iltimos keyinroq qayta urinib koʻring.',
			)
		} finally {
	...
	const filteredTables = useMemo(() => {
		return tables
			...
			.filter(item => {
				...
				const type =
					item.type === 'xona' || (item.capacity > 4 && !item.type)
						? 'xona'
						: 'stol'
	...
	const scrollToBook = (floorName?: string) => {
		if (floorName) {
			const dbFloor = activeFloors.find(
				f =>
					f.toLowerCase().includes(floorName.toLowerCase()) ||
					floorName.toLowerCase().includes(f.toLowerCase()),
			)
		...
		<main className='min-h-screen bg-white dark:bg-[#020617] text-slate-900 dark:text-white transition-colors duration-300'>
			...
			>
				...
				<div className='relative z-10 flex flex-col items-center justify-center px-6 text-center text-white max-w-4xl'>
					...
					>
						Anʼanaviy mehmondoʻstlik va yuksak nafosat uygʻunligi. Kafolatlangan
						va shinam kechalar uchun stol yoki VIP xonani hoziroq band qiling.
					</motion.p>
				...
				>
					<a
						href='#reservation'
						className='flex flex-col items-center gap-2 text-white/50 transition hover:text-white'
					>
			...
			<section
				id='reservation'
				className='relative px-6 py-24 sm:py-32 w-full max-w-7xl mx-auto'
			>
				<div className='mx-auto max-w-5xl'>
					<motion.div {...fadeUp} className='mb-14 text-center space-y-3'>
						<span className='text-xs uppercase font-black tracking-[0.35em] text-red-600'>
							Buyurtma berish
						</span>
						<h2 className='font-display text-4xl font-black uppercase tracking-tight sm:text-5xl md:text-6xl'>
							Stol band qilish
						</h2>
						<p className='mx-auto mt-4 max-w-xl text-slate-500 dark:text-slate-400 font-medium'>
							Uch oddiy qadamda restoranimizdan shaxsiy joyingizni band qiling.
							Savollaringiz bo'lsa biz doimo xizmatingizdamiz.
						</p>
					</motion.div>
                    ...
                    `}
									...
									{n < 3 && (
										<div
											className={`mx-3 h-[2px] flex-1 rounded-full transition-all duration-500
                    ...
                    `}
						...
						<div className='mb-10 flex justify-between text-[11px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 max-w-xl mx-auto px-1'>
							<span
								className={step === 1 ? 'text-red-500 dark:text-red-400' : ''}
							>
								Sana va vaqt
							</span>
							<span
								className={step === 2 ? 'text-red-500 dark:text-red-400' : ''}
							>
								Xona va Stol
							</span>
							<span
								className={step === 3 ? 'text-red-500 dark:text-red-400' : ''}
							>
								Maʻlumotlar
							</span>
						</div>
											...
											<div className='space-y-3'>
												...
												<div className='flex items-center justify-between rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/20 p-2.5 h-16'>
													...
													<div className='flex items-center gap-2'>
														...
														<span className='w-12 text-center text-xl font-black tabular-nums'>
															{guestCount}
														</span>
														<button
															type='button'
															onClick={() =>
																setGuestCount(Math.min(40, guestCount + 1))
															}
															className='grid h-10 w-10 place-items-center rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 transition hover:border-red-500/60 font-black text-red-500 text-lg shadow-sm'
														>
										...
										<div className='bg-slate-50/50 dark:bg-slate-950/30 rounded-3xl p-6 border border-slate-100 dark:border-slate-800/80'>
											<div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4'>
												{filteredTables.map(item => {
													...
													const isOccupied =
														!item.is_available || isVenueBooked(item.id)
													const isVIP = !!(
														item.name.toLowerCase().includes('vip') ||
														(item.floor && item.floor.includes('VIP'))
													)
													const isRoom =
														item.type === 'xona' ||
														(item.capacity > 4 && !item.type)
                              ...
                              `}
														>
															<div className='relative'>
																...
																<div
																	className={`absolute top-1/2 left-1/2 -translate-x-1/2 translate-y-[10px] z-20 min-w-[58px] px-2.5 py-1 rounded-lg text-white font-black text-[10px] shadow-lg truncate max-w-[85px] border border-white/20
											...
											{filteredTables.length === 0 && (
												<div className='py-16 text-center flex flex-col items-center justify-center gap-2 text-slate-400 dark:text-slate-500'>
													...
													<span className='font-black uppercase tracking-widest text-xs'>
														Bu filtrga mos keladigan joylar mavjud emas
													</span>
												</div>
											)}
										...
										<div className='space-y-4'>
											...
											<div>
												...
												<div className='flex items-stretch overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30 focus-within:border-red-500 transition-all'>
													<span className='grid place-items-center border-r border-slate-200 dark:border-slate-800/80 bg-slate-100/50 dark:bg-slate-950/40 px-4 text-sm font-bold text-slate-400'>
														+998
													</span>
													<input
														type='tel'
									...
									<aside className='rounded-3xl border border-slate-200/80 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-950/20 p-6 space-y-5 h-fit'>
										...
										<div className='divide-y divide-slate-100 dark:divide-slate-800/80 text-sm'>
											...
											<div className='flex justify-between py-3'>
												...
												<span className='font-bold'>
													{selectedSeating?.name} ({selectedSeating?.floor})
												</span>
											</div>
											<div className='flex justify-between py-3'>
												<span className='text-slate-400 font-medium'>
											...
											{selectedSeating?.price_per_hour ? (
												<div className='flex justify-between py-3'>
													...
													<span className='font-black text-red-500'>
														{selectedSeating.price_per_hour.toLocaleString()}{' '}
														soʻm
													</span>
												</div>
											) : null}
										...
										<div className='rounded-2xl bg-amber-50 dark:bg-amber-950/20 border border-amber-500/20 p-3.5 flex items-start gap-2.5'>
											...
											<p className='text-[10px] font-bold text-amber-600 dark:text-amber-500/90 leading-normal'>
												Restoranimiz qoidalari bo'yicha bron qilingan vaqtdan
												keyin 15 daqiqadan ko'p kechikilsa, buyurtma bekor
												qilinishi mumkin.
											</p>
										</div>
									</aside>
			...
			<section id='rooms' className='px-6 py-24 bg-white dark:bg-slate-950/40'>
				<div className='w-full max-w-7xl mx-auto'>
					<motion.div {...fadeUp} className='mb-14 text-center space-y-3'>
						...
						<p className='mx-auto mt-4 max-w-xl text-slate-500 dark:text-slate-400 font-medium'>
							Sizga va mehmonlaringizga mos keladigan o'ziga xos dizayn va
							atmosferaga ega zallarimiz bilan tanishing.
						</p>
					</motion.div>
					...
					<div className='p-8 text-center space-y-6'>
						...
						<div className='space-y-2'>
							...
							<p className='text-sm font-medium text-slate-400'>
								Restoranimizda sizni intiqlik bilan kutamiz
							</p>
						</div>
						...
						<div className='rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30 p-5 text-left text-sm space-y-2.5'>
							<div className='flex justify-between'>
								<span className='text-slate-400 font-medium'>Sana</span>
								<span className='font-bold'>
									{successBooking.date.toLocaleDateString('uz-UZ', {
										day: 'numeric',
										month: 'long',
										year: 'numeric',
									})}
