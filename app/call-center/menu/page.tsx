...
export default function MenuPage() {
	...
	const fetchItems = async () => {
		...
		try {
			...
			if (error) {
				console.error('[call-center] Menu fetch error:', error)
				throw error
			}
			...
			setItems([])
			toast.error(
				'Menyuni yuklashda xatolik: ' + (err.message || "Noma'lum xato"),
			)
		} finally {
			setLoading(false)
		}
	...
	const fetchCategories = async () => {
		try {
			...
			if (error) {
				console.error('[call-center] Categories fetch error:', error)
				return
			}
						...
						<button
							...
							className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wider transition-all ${
								selectedCategory === null
									...
									: 'bg-white dark:bg-[#111827] text-slate-500 hover:text-slate-800 dark:text-slate-400 border border-slate-200 dark:border-slate-700'
							}`}
						>
							Hammasi
						</button>
