...
const KITCHEN_STEPS = [
	{
		...
		description: 'Operator tomonidan tasdiqlandi',
	},
	{
		id: 'tayyorlanmoqda',
		label: 'Tayyorlanmoqda',
...
const DELIVERY_STEPS = [
	{
		id: "yo'lda",
		label: "Yo'lda",
		description: "Kuryer yetkazish uchun yo'lda",
	},
	{
		id: 'yetkazildi',
		label: 'Yetkazildi',
		...
		try {
			...
			if (data) {
				const newOrder: Order = {
					...
					phone: data.phone || '-',
					address:
						data.address || data.delivery_address || "Manzil ko'rsatilmagan",
					items: data.items || [],
					...
					payload => {
						...
						if (payload.new.id === orderId) {
							const updatedOrder: Order = {
								id: payload.new.id,
								order_number:
									payload.new.order_number || payload.new.id.slice(-6),
	...
	const updateStatus = async (newStatus: OrderStatus) => {
		...
		try {
			...
			if (newStatus === 'olib_ketildi' && !order.courier_id) {
				updateData.courier_id =
					'courier_' + Math.random().toString(36).substr(2, 9)
			...
			<div className='min-h-screen bg-[#f5f7fa] dark:bg-[#0a0a0a] flex items-center justify-center'>
				<div className='text-center'>
					...
					<Link
						...
						className='text-[#FF0000] hover:underline mt-2 inline-block'
					>
						Orqaga qaytish
					</Link>
				</div>
							...
							<div className='space-y-3 mb-6'>
								...
								{order.status === 'yangi' ? (
									...
									<motion.button
										...
										onClick={() =>
											canUpdateKitchenStatus
												? updateStatus('qabul_qilindi')
												: handleUnauthorizedClick('kitchen')
										}
										disabled={
											!canUpdateKitchenStatus || updating === 'qabul_qilindi'
										}
										className={cn(
									...
									KITCHEN_STEPS.map((step, index) => {
										...
										const isNextStep =
											index === kitchenStepIndex + 1 && !isCompleted
										...
										<div className='relative h-[350px] bg-gray-50 dark:bg-gray-800/50 rounded-2xl overflow-hidden flex items-center justify-center'>
											<div className='text-center p-6'>
												...
												<p className='text-gray-600 dark:text-gray-400 font-medium'>
													Kuryer hali yo'lga chiqmagan
												</p>
												<p className='text-sm text-gray-400 mt-1'>
													Buyurtma tayyor, kuryer kutilyapti
												</p>
											</div>
										</div>
									)}
