...
export default function ProfessionalCallCenter() {
	...
	// 5. Buyurtmani yuborish mantiig'i
	const handleSubmit = async () => {
		...
		setSubmitting(true)
		try {
			// Supabase jadvalingizdagi ustunlarga (customer_name, delivery_address, items, etc) mos payload
			const result = await submitOrder({
				customer_name: customerName,
				phone,
				type: mode === 'delivery' ? 'delivery' : 'dine_in',
				delivery_address:
					mode === 'delivery' ? address : `Stol: ${tableNumber}`,
				// Items ustuni uchun JSON formatda yuboramiz
				items: cart.map(c => ({
					id: c.item.id,
					name: c.item.name,
					price: c.item.price,
		...
		<div className='min-h-screen bg-[#F8FAFC] dark:bg-slate-950 text-slate-900 dark:text-slate-50'>
			...
			<header className='h-20 border-b bg-white dark:bg-slate-900 sticky top-0 z-50 px-8 flex items-center justify-between shadow-sm'>
				...
				<div className='flex items-center gap-4'>
					...
					<Button
						variant='ghost'
						className='font-bold gap-2 text-slate-600 dark:text-slate-400 hover:text-red-500'
									...
									<div className='grid grid-cols-2 lg:grid-cols-3 gap-4'>
										{filteredItems.map(item => (
											<button
												...
												className='p-5 bg-white dark:bg-slate-900 rounded-3xl border border-transparent hover:border-red-500/50 hover:shadow-xl hover:shadow-red-500/5 transition-all text-left flex items-center justify-between group active:scale-[0.98]'
											>
					...
					<Card className='rounded-[2.5rem] border-none shadow-2xl shadow-slate-200/50 dark:shadow-none bg-white dark:bg-slate-900 sticky top-28 overflow-hidden'>
						<CardHeader className='p-8 pb-4'>
							<CardTitle className='text-xl font-black flex items-center justify-between'>
								YANGI BUYURTMA
								<span className='text-[10px] bg-red-50 text-red-500 px-3 py-1 rounded-full animate-pulse'>
									LIVE
								</span>
							</CardTitle>
						</CardHeader>
							...
							<div className='space-y-2'>
								...
								{mode === 'delivery' ? (
									<Textarea
										...
										className='rounded-2xl bg-slate-50 dark:bg-slate-800 border-none font-bold min-h-[80px]'
									/>
								) : (
									<Input
										value={tableNumber}
