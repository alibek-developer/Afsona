...
export function MenuItemDialog({
	...
	// Kategoriyalarni yuklash - faqat dialog ochilganda bir marta
	useEffect(() => {
		if (!open) return
	...
	// Formani to'ldirish yoki tozalash - faqat open va editItem?.id o'zgargandagina ishlaydi
	useEffect(() => {
		if (open && editItem?.id !== undefined) {
			if (editItem) {
				setFormData({
					name: editItem.name || '',
					description: editItem.description || '',
					price: editItem.price?.toString() || '',
					category: editItem.category?.toString() || '',
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const itemData = {
			...
			is_available:
				formData.available_on_website || formData.available_on_mobile,
		}
	...
	const toggleSwitch = useCallback(
		(id: 'available_on_website' | 'available_on_mobile') => {
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent
				className='max-w-[440px] max-h-[90vh] overflow-hidden p-0 border-none rounded-[2rem] bg-white dark:bg-slate-900 shadow-2xl flex flex-col'
				onOpenAutoFocus={e => e.preventDefault()}
				onCloseAutoFocus={e => e.preventDefault()}
			>
				{/* Header - Sticky */}
				<div className='bg-slate-900 dark:bg-black p-5 text-white flex items-center gap-4 shrink-0'>
				...
				>
					...
					<div className='space-y-1.5'>
						...
						<div
							className={`relative h-44 w-full rounded-2xl border-2 border-dashed transition-all flex items-center justify-center overflow-hidden ${formData.image_url ? 'border-solid border-indigo-500' : 'border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 hover:border-indigo-500'}`}
						>
							{formData.image_url ? (
								<>
									<Image
									...
									<div className='absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity flex items-center justify-center'>
										<Label
											...
											className='cursor-pointer bg-white text-black px-4 py-2 rounded-lg text-xs font-bold uppercase hover:bg-slate-100'
										>
											O'zgartirish
										</Label>
									</div>
					...
					<div className='space-y-1.5'>
						...
						<Textarea
							...
							placeholder='Taom tarkibi va tavsifi...'
						/>
					</div>
					...
					<div className='flex gap-2'>
						{[
							{ id: 'available_on_website' as const, label: 'Veb-sayt' },
							{ id: 'available_on_mobile' as const, label: 'Mobil' },
						].map(sw => (
							<div
								key={sw.id}
								className={`flex-1 flex items-center justify-between p-3 rounded-xl border cursor-pointer transition-all ${formData[sw.id] ? 'border-indigo-500 bg-indigo-50/50 dark:bg-indigo-500/10' : 'border-slate-100 dark:border-slate-800 bg-slate-50/50'}`}
							>
								<span className='text-[10px] font-bold uppercase text-slate-500'>
									{sw.label}
								</span>
								<Checkbox
									checked={formData[sw.id]}
									onCheckedChange={checked => {
										setFormData(prev => ({ ...prev, [sw.id]: checked }))
									}}
					...
					{/* Footer Buttons - Sticky position logic inside flex */}
					<div className='flex gap-3 pt-4 bg-white dark:bg-slate-900 sticky bottom-0 mt-auto'>
						<Button
							type='button'
							variant='ghost'
							onClick={() => onOpenChange(false)}
