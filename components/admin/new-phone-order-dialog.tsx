...
export function NewPhoneOrderDialog({
  open,
  onOpenChange,
}: NewPhoneOrderDialogProps) {
  const menuItems = useMenuStore((state) => state.items);
  const addOrder = useOrderStore((state) => state.addOrder);

  const [customerName, setCustomerName] = useState("");
  const [customerPhone, setCustomerPhone] = useState("");
  const [orderMode, setOrderMode] = useState<"dine-in" | "delivery">(
    "delivery",
  );
  ...
  const isValid =
    ...
    customerPhone &&
    orderItems.length > 0 &&
    (orderMode === "dine-in" ? tableNumber : deliveryAddress);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Call-center orqali yangi buyurtma qo'shish
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Select
              value={paymentMethod}
              onValueChange={(v) =>
                setPaymentMethod(v as Order["payment_method"])
              }
            >
